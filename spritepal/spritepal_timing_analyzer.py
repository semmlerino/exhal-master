...
try:
    ...
    from core.mmap_rom_reader import CachedROMReader, MemoryMappedROMReader
...
class SpritePalTimingAnalyzer:
    ...
    def _compare_rom_access_methods(self) -> dict[str, Any]:
        ...
        for pattern_name, test_func in access_patterns:
            ...
            for reader_type in ['mmap_reader', 'cached_reader']:
                ...
                for _i in range(5):  # Fewer iterations for complex patterns
                    try:
                        start_time = time.perf_counter()
    ...
    def _analyze_threading_behavior(self) -> dict[str, Any]:
        ...
        threading_analysis["async_cache_behavior"] = {
            ...
            "data_consistency": len({r.get("data_hash", "") for r in request_results.values() if r["success"]}) <= num_requests
        }
    ...
    def _analyze_hal_timing(self) -> dict[str, Any]:
        ...
        try:
            ...
            for _i in range(3):  # Limited iterations due to potential expense
                start_time = time.perf_counter()
    ...
    def _test_single_read(self, size: int) -> bytes | None:
    ...
    def _test_multiple_reads(self, sizes: list[int]) -> bytes | None:
