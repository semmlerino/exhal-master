...
def scan_rom_for_sprites(rom_path: str, start: int = 0x40000, end: int = 0x200000) -> Dict[int, bytes]:
    ...
    print(f"Building sprite database from ROM...")
...
def match_ram_to_rom(ram_data: bytes, sprite_db: Dict[int, bytes]) -> Optional[int]:
    """
...
def quick_find_sprite(rom_path: str, ram_dump: str) -> Optional[int]:
    """
...
def main():
    ...
    if sys.argv[2] == "--session":
        ...
        if rom_offset:
            ...
            print(f"This is the compressed sprite location in the ROM.")
        else:
