...
# Import fixes
IMPORT_REPLACEMENTS = [
    (r'from main_window import SpritePalMainWindow',
     'from ui.main_window import SpritePalMainWindow'),
    (r'import main_window',
    ...
    try:
        ...
        if content != original_content:
            if dry_run:
                ...
                if import_changes:
                    print(f"  - Import fixes: {import_changes}")
                if ignore_changes:
    ...
    if args.specific_files:
        ...
        for pattern in ['**/*.py']:
            for file_path in args.path.glob(pattern):
                ...
                if any(part in ['.venv', 'venv', '__pycache__'] 
