    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name):
            if node.func.id in ['QApplication', 'QWidget', 'QDialog', 'QMainWindow']:
                self.qt_widgets_created.append({
                    'type': node.func.id,
        ...
        for i, line in enumerate(lines):
            ...
            if 'def test_' in line and 'parent_widget' in line and 'qtbot' not in line:
                modified_line = re.sub(
                    r'def (test_\w+)\(self, parent_widget\):',
                    r'def \1(self, qtbot):',
                    line
                )
            ...
            if modified_line != line or (
                'parent_widget' in line and
                any(widget in line for widget in ['QWidget()', 'QDialog(', 'QMainWindow('])
            ):
...
def main():
    ...
    if args.fix:
        ...
        if args.file:
            ...
            files_to_process = [
                Path(file_path) for file_path, result in migrator.analysis_results.items()
                if result.get('needs_migration', False)
            ]
    ...
    if args.verify:
        ...
        still_need_migration = sum(
            1 for r in migrator.analysis_results.values()
