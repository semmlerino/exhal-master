        ...
        for i, line in enumerate(lines, 1):
            if '.connect(' in line:
                ...
                if 'lambda' in line and 'QMessageBox' in line:
                    extra_issues.append(
                        f"Line {i}: Avoid creating GUI objects in lambda connected to signals"
                    )

        # Check for Qt boolean evaluation issues
        for i, line in enumerate(lines, 1):
            # Look for if statements with Qt objects
            if line.strip().startswith('if ') and any(qt_class in line for qt_class in
                ['QLayout', 'QWidget', 'QTabWidget', 'QListWidget']):
                if ' is not None' not in line and ' is None' not in line:
                    extra_issues.append(
                        f"Line {i}: Use 'is not None' instead of truthiness for Qt objects"
                    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-sort-imports', action='store_true',
                       help='Skip import sorting')
