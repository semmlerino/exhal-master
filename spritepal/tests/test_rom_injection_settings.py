...
class TestROMInjectionSettingsPersistence:
    ...
    def settings_manager(self, temp_settings_file):
        ...
        patcher = patch.object(SettingsManager, '_get_settings_file',
                              return_value=Path(temp_settings_file))
        patcher.start()
    ...
    def test_save_rom_injection_parameters(self, mock_dialog, settings_manager):
        ...
        # Patch get_logger to avoid logger import issues
        with patch('spritepal.ui.injection_dialog.get_logger') as mock_get_logger:
            mock_logger = Mock()
            mock_get_logger.return_value = mock_logger
            
            # Save parameters
            with patch('spritepal.ui.injection_dialog.get_settings_manager',
                       return_value=settings_manager):
                mock_dialog.save_rom_injection_parameters()

        # Verify saved values
        assert settings_manager.get_value(SETTINGS_NS_ROM_INJECTION, 
                                         SETTINGS_KEY_LAST_INPUT_ROM) == "/path/to/test.sfc"
        assert settings_manager.get_value(SETTINGS_NS_ROM_INJECTION,
                                         SETTINGS_KEY_LAST_SPRITE_LOCATION) == "Kirby Sprite (0x123456)"
        assert settings_manager.get_value(SETTINGS_NS_ROM_INJECTION,
                                         SETTINGS_KEY_LAST_CUSTOM_OFFSET) == "0x123456"
        assert settings_manager.get_value(SETTINGS_NS_ROM_INJECTION,
                                         SETTINGS_KEY_FAST_COMPRESSION) is True
    ...
    def test_save_empty_rom_injection_parameters(self, mock_dialog, settings_manager):
        ...
        value = settings_manager.get_value(SETTINGS_NS_ROM_INJECTION,
                                          SETTINGS_KEY_LAST_CUSTOM_OFFSET, None)
        assert value == ""  # Empty string is saved
        
        # Verify fast compression is saved as False
        value = settings_manager.get_value(SETTINGS_NS_ROM_INJECTION,
                                          SETTINGS_KEY_FAST_COMPRESSION, None)
    ...
    def test_save_vram_injection_paths(self, settings_manager):
        ...
        # Save VRAM paths
        settings_manager.set_value(SETTINGS_NS_ROM_INJECTION,
                                  SETTINGS_KEY_LAST_INPUT_VRAM, "/path/to/input.dmp")
        settings_manager.set_value(SETTINGS_NS_ROM_INJECTION,
                                  SETTINGS_KEY_LAST_OUTPUT_VRAM, "/path/to/output.dmp")
        settings_manager.save()

        # Create a new settings manager to verify persistence
        with patch.object(SettingsManager, '_get_settings_file',
                         return_value=settings_manager._settings_file):
            new_manager = SettingsManager()
            assert new_manager.get_value(SETTINGS_NS_ROM_INJECTION,
                                        SETTINGS_KEY_LAST_INPUT_VRAM) == "/path/to/input.dmp"
            assert new_manager.get_value(SETTINGS_NS_ROM_INJECTION,
                                        SETTINGS_KEY_LAST_OUTPUT_VRAM) == "/path/to/output.dmp"
    ...
    def test_load_rom_injection_defaults(self, mock_dialog, settings_manager):
        ...
        settings_manager.set_value(SETTINGS_NS_ROM_INJECTION,
                                  SETTINGS_KEY_LAST_INPUT_ROM, "/test/rom.sfc")
        settings_manager.set_value(SETTINGS_NS_ROM_INJECTION,
                                  SETTINGS_KEY_LAST_SPRITE_LOCATION, "Kirby Sprite (0x123456)")
        settings_manager.set_value(SETTINGS_NS_ROM_INJECTION,
                                  SETTINGS_KEY_LAST_CUSTOM_OFFSET, "0x789ABC")
        settings_manager.set_value(SETTINGS_NS_ROM_INJECTION,
                                  SETTINGS_KEY_FAST_COMPRESSION, True)

        # Patch get_logger first
        with patch('spritepal.ui.injection_dialog.get_logger') as mock_get_logger:
            mock_logger = Mock()
            mock_get_logger.return_value = mock_logger
            ...
            with patch('os.path.exists', return_value=True), \
                 patch('spritepal.ui.injection_dialog.get_settings_manager',
                       return_value=settings_manager):
    ...
    def test_settings_save_error_handling(self, mock_dialog, settings_manager, caplog):
        ...
        with patch('spritepal.ui.injection_dialog.get_settings_manager',
                   return_value=settings_manager), \
             patch.object(settings_manager, 'save', side_effect=IOError("Permission denied")):
    ...
    def test_sprite_location_restoration_after_rom_load(self, mock_dialog):
        ...
        with patch('spritepal.ui.injection_dialog.get_logger') as mock_get_logger:
            ...
            with patch('spritepal.ui.injection_dialog.get_settings_manager') as mock_get_settings:
                mock_settings = Mock()
    ...
    def test_settings_namespace_consistency(self, settings_manager):
        ...
        settings_manager.set_value(SETTINGS_NS_ROM_INJECTION,
                                  SETTINGS_KEY_LAST_INPUT_ROM, "/test.sfc")
        settings_manager.set_value(SETTINGS_NS_ROM_INJECTION,
                                  SETTINGS_KEY_LAST_INPUT_VRAM, "/test.dmp")
        settings_manager.save()
        ...
        assert "injection" not in raw_settings or \
               SETTINGS_KEY_LAST_INPUT_ROM not in raw_settings.get("injection", {})