...
from core.hal_compression import (
    HALProcessPool, HALCompressor, HALRequest, HALResult,
    HALCompressionError, HALPoolError, _hal_worker_process
)
...
class TestHALProcessPoolPerformance:
    ...
    def test_force_reset_performance(self, mock_tools):
        ...
        with patch('multiprocessing.Manager') as mock_manager, \
            ...
            start_time = time.time()
            pool1.shutdown()
            normal_shutdown_time = time.time() - start_time
            ...
            start_time = time.time()
            pool2.force_reset()
            force_reset_time = time.time() - start_time
