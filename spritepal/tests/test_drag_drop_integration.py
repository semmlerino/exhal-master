...
class TestDragDropIntegration:
    ...
    def test_single_file_drag_drop(self, sample_files):
        ...
        drag_enter, drop_event = self.create_mock_drag_drop_events([sample_files["files"]["vram"]])
    ...
    def test_multiple_file_drag_drop(self, sample_files):
        ...
        # Verify call args
        drop_zone.file_dropped.emit.assert_called_with(str(sample_files["files"]["vram"]))
    ...
    def test_invalid_file_drag_drop(self, sample_files):
        ...
        drag_enter, drop_event = self.create_mock_drag_drop_events([sample_files["files"]["nonexistent"]])
        result = drop_zone.dropEvent(drop_event)
        ...
        drag_enter, drop_event = self.create_mock_drag_drop_events([sample_files["files"]["invalid"]])
        result = drop_zone.dropEvent(drop_event)
        assert result is True
    ...
    def test_drag_drop_with_existing_files(self, sample_files):
        ...
        drag_enter, drop_event = self.create_mock_drag_drop_events([sample_files["files"]["vram_backup"]])
        result = drop_zone.dropEvent(drop_event)
        assert result is True
        ...
        drop_zone.file_dropped.emit.assert_called_with(str(sample_files["files"]["vram_backup"]))
...
class TestExtractionPanelIntegration:
    ...
    def test_extraction_panel_drag_drop_workflow(self, sample_files):
        ...
        panel.cgram_drop.get_file_path.return_value = str(sample_files["files"]["cgram"])
