...
from tests.infrastructure.mock_hal import MockHALProcessPool, MockHALCompressor
...
class TestHALMockingPerformance:
    ...
    def test_mock_pool_initialization_speed(self, benchmark):
        ...
        result = benchmark(init_mock_pool)
    ...
    def test_real_vs_mock_performance_comparison(self, request, tmp_path):
        ...
        start = time.perf_counter()
        real_result = real_pool.submit_request(request)
        ...
        start = time.perf_counter()
        mock_result = mock_pool.submit_request(request)
