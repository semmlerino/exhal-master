...
class TestRealMainWindowStateIntegration:
    ...
    def test_real_signal_propagation_vs_mocked_signals(self):
        ...
        with qt_widget_test(MainWindow) as main_window:
            ...
            extract_spy = QSignalSpy(main_window.extract_requested)
            ...
            inject_spy = QSignalSpy(main_window.inject_requested)
    ...
    def test_real_extraction_parameter_gathering_vs_mocked(self):
        ...
        with qt_widget_test(MainWindow) as main_window:
            ...
            # REAL BEHAVIOR: output_base comes from _output_path, not UI field
            # This is how MainWindow actually works vs mocked assumptions
            assert params["output_base"] == main_window._output_path, "output_base should match _output_path"
...
class TestBugDiscoveryRealVsMocked:
    ...
    def test_discovered_bug_widget_state_synchronization(self):
        ...
        with qt_widget_test(MainWindow) as main_window:
            ...
            initial_extract_enabled = main_window.extract_button.isEnabled()
            initial_editor_enabled = main_window.open_editor_button.isEnabled()
            
            # Simulate state change
            main_window.extraction_complete(["sync_test.png"])
            
            after_extract_enabled = main_window.extract_button.isEnabled()
    ...
    def test_discovered_bug_button_enabled_but_no_action(self):
        ...
        with qt_widget_test(MainWindow) as main_window:
            ...
            # Call extraction_complete() - this enables button but doesn't set _output_path
            main_window.extraction_complete(["test.png"])
            
            # EXPOSED BUG: Button is enabled but _output_path is still empty
            assert main_window.open_editor_button.isEnabled() is True, "Button should be enabled"
            assert main_window._output_path == "", "BUG: _output_path is empty but button is enabled"
            
            # Click the enabled button - it should emit signal but WON'T due to bug
            QTest.mouseClick(main_window.open_editor_button, Qt.MouseButton.LeftButton)
            self.qt_app.processEvents()
            
            # REAL BUG EXPOSED: Button is enabled but no signal emitted!
            assert len(signal_spy) == 0, "BUG: Enabled button clicked but no signal emitted due to empty _output_path"
            
            # Show that fixing _output_path makes the button work
            main_window._output_path = "test_sprites"
            QTest.mouseClick(main_window.open_editor_button, Qt.MouseButton.LeftButton)
            self.qt_app.processEvents()
            
            # Now signal should be emitted
            assert len(signal_spy) == 1, "FIXED: Signal emitted when _output_path is set"
