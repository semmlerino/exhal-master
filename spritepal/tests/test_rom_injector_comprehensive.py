...
class TestROMHeader:
    ...
    def test_rom_header_equality(self):
        ...
        header1 = ROMHeader(
            title="TEST", rom_type=0x20, rom_size=0x08, sram_size=0x00,
            checksum=0x1234, checksum_complement=0xEDCB, header_offset=0
        )
        header2 = ROMHeader(
            title="TEST", rom_type=0x20, rom_size=0x08, sram_size=0x00,
            checksum=0x1234, checksum_complement=0xEDCB, header_offset=0
        ...
        header3 = ROMHeader(
            title="DIFFERENT", rom_type=0x20, rom_size=0x08, sram_size=0x00,
            checksum=0x1234, checksum_complement=0xEDCB, header_offset=0
        )
...
class TestROMInjectorInit:
    ...
    def test_rom_injector_init(self):
        ...
        assert injector.KIRBY_SPRITE_POINTERS is not None
        assert "kirby_normal" in injector.KIRBY_SPRITE_POINTERS
...
class TestROMInjectorReadHeader:
    ...
    def test_read_rom_header_invalid_checksum(self):
        ...
        try:
            injector = ROMInjector()
            with pytest.raises(ValueError, match="Could not find valid SNES ROM header"):
...
class TestROMInjectorChecksum:
    ...
    def test_calculate_checksum_basic(self):
        ...
        injector.header = ROMHeader(
            title="TEST", rom_type=0x20, rom_size=0x08, sram_size=0x00,
            checksum=0, checksum_complement=0, header_offset=0
        )
    ...
    def test_calculate_checksum_with_smc_header(self):
        ...
        injector.header = ROMHeader(
            title="TEST", rom_type=0x20, rom_size=0x08, sram_size=0x00,
            checksum=0, checksum_complement=0, header_offset=512
        )
...
class TestROMInjectorCompression:
    ...
    def test_find_compressed_sprite_success(self, mock_unlink, mock_temp):
        ...
        injector.hal_compressor.decompress_from_rom.assert_called_once_with("/tmp/test_rom", offset)
    ...
    def test_find_compressed_sprite_decompression_error(self, mock_unlink, mock_temp):
        ...
        injector = ROMInjector()
        injector.hal_compressor = MagicMock()
        injector.hal_compressor.decompress_from_rom.side_effect = HALCompressionError("Decompression failed")
...
class TestROMInjectorSpriteInjection:
    ...
    def create_test_rom_data(self, title="TEST ROM", rom_type=0x20):
        ...
        rom_data = bytearray(0x20000)  # 128KB - definitely large enough for any header location
    ...
    def test_inject_sprite_to_rom_success(self, mock_unlink, mock_temp):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                injector.find_compressed_sprite = MagicMock(return_value=(0x1000, b"original_data"))
                injector.hal_compressor = MagicMock()
    ...
    def test_inject_sprite_to_rom_sprite_too_large(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            injector.convert_png_to_4bpp = MagicMock(return_value=b"4bpp_data")
            injector.find_compressed_sprite = MagicMock(return_value=(0x800, b"original_data"))
            injector.hal_compressor = MagicMock()
            injector.hal_compressor.compress_to_file = MagicMock(return_value=0x1000)  # Too large
    ...
    def test_inject_sprite_to_rom_compression_error(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            injector.find_compressed_sprite = MagicMock(return_value=(0x1000, b"original_data"))
            injector.hal_compressor = MagicMock()
            injector.hal_compressor.compress_to_file.side_effect = HALCompressionError("Compression failed")
    ...
    def test_inject_sprite_to_rom_file_not_found(self):
        ...
        success, message = injector.inject_sprite_to_rom(
            "/nonexistent/sprite.png", "/nonexistent/rom.sfc", "/tmp/output.sfc", 0x1000, False
        )
...
class TestROMInjectorSpriteLocations:
    ...
    def test_find_sprite_locations_pointer_calculation(self):
        ...
        kirby_normal = locations["kirby_normal"]
        assert kirby_normal.offset == 0x0C8000
        assert kirby_normal.bank == 0x0C
        assert kirby_normal.address == 0x8000
...
class TestROMInjectionWorker:
    ...
    def test_rom_injection_worker_run_success(self, mock_injector_class):
        ...
        mock_injector.read_rom_header.return_value = ROMHeader(
            title="TEST ROM", rom_type=0x20, rom_size=0x08, sram_size=0x00,
            checksum=0x1234, checksum_complement=0xEDCB, header_offset=0
        )
    ...
    @patch('spritepal.core.rom_injector.ROMInjector')
    def test_rom_injection_worker_run_tools_error(self, mock_injector_class):
        ...
        mock_injector.validate_sprite.return_value = (True, "Valid sprite")
        mock_injector.hal_compressor.test_tools.return_value = (False, "Tools not found")
    ...
    def test_rom_injection_worker_run_exception(self, mock_injector_class):
        ...
        worker.finished.emit.assert_called_once_with(False, "Unexpected error: Unexpected error")
