-- Mesen 2 Sprite Finder - Working Version with Correct API
-- Uses the verified Mesen 2 API with proper enums and types

-- First, let's enumerate emu.callbackType to see what's available
emu.log("=== Mesen 2 Sprite Finder (Working Version) ===")
if emu.callbackType then
    emu.log("Callback types available:")
...
local function read_dma_channel(channel)
    ...
    -- Read all DMA registers using correct memory type
...
local function update_obsel_from_state()
    local state_data = emu.getState()
    if state_data and state_data.ppu then
        -- Read OBSEL register directly from memory
...
-- Analyze OAM using correct SNES sprite RAM type
local function analyze_oam()
    -- Read OAM data using the correct memory type for SNES
    local oam_data = {}
    ...
    for i = 0, 127 do
        ...
        if y < 224 or y >= 240 then  -- On-screen or wrapped
            ...
            if state.obsel_config then
                ...
                -- Adjust for name table selection
                if sprite.name_table ~= 0 and state.obsel_config.name_select > 0 then
                    name_base = name_base + (state.obsel_config.name_select * 0x1000)
                end
                
                sprite.vram_addr = name_base + (tile * 32)  -- 32 bytes per tile
...
local function debug_vram_contents()
    if state.obsel_config then
        ...
        -- Read first few bytes of sprite pattern table from VRAM
        local sample = {}
        for i = 0, 15 do
            sample[i] = emu.read(sprite_base + i, emu.memType.snesVideoRam)
        end
...
function export_findings()
    ...
    table.sort(sorted_offsets, function(a, b) return a.data.hit_count > b.data.hit_count end)
    ...
    for _, entry in ipairs(sorted_offsets) do
        ...
        for vram in pairs(entry.data.vram_addrs) do
            ...
            if vram_count <= 3 then
                table.insert(vram_list, string.format("$%04X", vram))
            end
        end
        
        output = output .. string.format(
            "  ROM $%06X: %d hits, frames %d-%d, VRAM: %s%s\n",
            entry.offset,
...
function export_json()
    ...
    -- Add ROM offsets sorted by frequency
    for offset, data in pairs(state.unique_rom_offsets) do
    ...
    table.sort(json_data.rom_offsets, function(a, b) return a.hits > b.hits end)
...
-- Initialize
function init()
    emu.log("Starting sprite finder with correct API...")
    emu.log("Using: emu.memType.snesMemory, emu.memType.snesSpriteRam, emu.eventType.endFrame")
    
    -- Register callbacks with correct API
    -- Use "write" string or integer 2 (both work)
    state.callbacks.dma = emu.addMemoryCallback(
        on_dma_enable_write,
        "write",  -- or use integer 2
        DMA_ENABLE,
        DMA_ENABLE
    )
    ...
    -- Use emu.eventType.endFrame which equals 3
    ...
    emu.log("Callbacks registered successfully!")
    emu.log("Monitoring DMA transfers and sprites...")
