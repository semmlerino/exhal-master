...
class SpriteInjector:
    ...
    def validate_sprite(self, sprite_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if width % 8 != 0 or height % 8 != 0:
                return False, f"Image dimensions must be multiples of 8 (found {width}x{height})"
            ...
            return False, f"Error validating sprite: {str(e)}"
    ...
    def inject_sprite(self, sprite_path: str, vram_path: str, output_path: str, 
                     offset: Optional[int] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            if offset + len(tile_data) > len(self.vram_data):
                return False, f"Tile data ({len(tile_data)} bytes) would exceed VRAM size at offset 0x{offset:04X}"
            ...
            return True, f"Successfully injected {len(tile_data)} bytes at offset 0x{offset:04X}"
            
        except Exception as e:
            return False, f"Error injecting sprite: {str(e)}"
    ...
    def get_extraction_info(self) -> Optional[dict]:
        ...
        if self.metadata and 'extraction' in self.metadata:
            return self.metadata['extraction']
        return None
...
class InjectionWorker(QThread):
    ...
    def __init__(self, sprite_path: str, vram_input: str, vram_output: str, 
                 offset: int, metadata_path: Optional[str] = None):
        super().__init__()
        self.sprite_path = sprite_path
