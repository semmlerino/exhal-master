...
class ROMInjectionWorker(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.progress_percent.emit(int((current_step / total_steps) * 100))
            ROMBackupManager.create_backup(self.rom_input)
            current_step += 1
            ...
            if success:
                ...
                logger.info("ROM injection worker completed successfully")
            else:
                logger.error(f"ROM injection worker failed: {message}")
