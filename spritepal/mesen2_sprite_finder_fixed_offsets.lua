-- Mesen 2 Enhanced Sprite Finder with Visual Feedback (FIXED OFFSETS)
-- Features: On-screen HUD, hotkey controls, better file management, visual feedback
-- FIX: Clear sprite ROM mappings each frame to show correct offsets for each sprite

emu.log("=== Enhanced Sprite Finder Starting (Fixed Offsets) ===")
emu.log("AUTO-CAPTURE ENABLED! Controls: 2=Pause, 3=Export, 4=Reset, 5=Toggle HUD, 6=Toggle Offsets")
...
local state = {
    -- Core tracking
    frame_count = 0,
    capture_active = true,  -- START CAPTURING IMMEDIATELY
    dma_captures = {},
    active_sprites = {},
    unique_rom_offsets = {},
...
-- Correlate sprites with DMA (FIXED: Now uses fresh correlations each frame)
local function correlate_sprites_and_dma()
    if not state.obsel_config or #state.dma_captures == 0 then return end
    
    -- FIX: Don't persist mappings between frames - recalculate fresh
    -- This ensures each sprite shows its current ROM offset, not a historical one
                ...
                if capture.frame >= state.frame_count - 60 then
                    if capture.vram_addr and capture.rom_offset then
                        ...
                        if dma_start < sprite_vram_end and dma_end > sprite_vram_start then
                            -- Map sprite to ROM offset (fresh mapping each frame)
                            state.sprite_rom_map[sprite.id] = capture.rom_offset
                            
                            -- Track statistics (only count first time we see this combination)
                            if not state.sprite_rom_map[sprite.id .. "_counted"] then
                                state.sprite_rom_map[sprite.id .. "_counted"] = true
        ...
        if sprite.x < 256 and sprite.y < 224 then
            ...
            if config.show_offsets and rom_offset then
                ...
                emu.drawString(text_x + 4, text_y + 4, offset_text, 
                    0xFF000000)  -- 0xFF prefix = opaque, black text
            end
        end
    end
end
...
local function on_frame_end()
    ...
    -- FIX: Clear sprite ROM mappings each frame
    -- This ensures sprites show their current ROM offset, not a stale one
    state.sprite_rom_map = {}
...
-- Initialize
function init()
    emu.log("Enhanced Sprite Finder initialized (Fixed Offsets)!")
    emu.log("AUTO-CAPTURING SPRITES! Press 2 to pause, 3 to export")
    ...
    add_message("AUTO-CAPTURE ACTIVE!", 300)
    add_message("2=Pause 3=Export 4=Reset 6=Offsets", 300)
end
