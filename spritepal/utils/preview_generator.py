...
class PreviewGenerator(QObject):
    ...
    def _process_pending_request(self) -> None:
        ...
        try:
            ...
            logger.exception("Error in debounced preview generation")  # TRY401: exception already logged
    ...
    def _generate_preview_impl(self,
        ...
        try:
            ...
            logger.exception(f"Preview generation failed for {request.source_type}:{request.offset:06X}")  # TRY401: exception already logged
            if progress_callback:
...
class _PreviewGeneratorSingleton:
    ...
    def cleanup(cls) -> None:
        ...
        This method safely cleans up and removes the global PreviewGenerator
        instance. It is thread-safe and can be called during application shutdown.

        Thread Safety:
            - Uses the same lock as get()
