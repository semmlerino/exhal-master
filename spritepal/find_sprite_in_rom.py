...
def decompress_at_offset(rom_path: str, offset: int, exhal_path: str = "./exhal") -> bytes | None:
    """
            ...
            if decompressed[:len(signature)] == signature:
                ...
                if len(decompressed) >= len(target_data):
                    if decompressed[:len(target_data)] == target_data:
                        ...
                        print("  Partial match only (signature matches but full data differs)")
...
def quick_sprite_search(rom_path: str, ram_address: int, sprite_size: int = 0x800) -> int | None:
    """
