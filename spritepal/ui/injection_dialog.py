...
class InjectionDialog(QDialog):
    ...
    def __init__(self, parent=None, sprite_path: str = "", metadata_path: str = "", input_vram: str = ""):
            ...
            try:
                ...
                if "extraction" in self.metadata:
                    ...
                    if source_type == "rom":
                        ...
                        info_text = f"Original ROM: {extraction.get('rom_source', 'Unknown')}\n"
                        info_text += f"Sprite: {extraction.get('sprite_name', 'Unknown')}\n"
                        info_text += f"ROM Offset: {extraction.get('rom_offset', 'Unknown')}\n"
                        ...
                        info_text = (
                            f"Original VRAM: {extraction.get('vram_source', 'Unknown')}\n"
                        )
    ...
    def _browse_sprite(self):
        ...
        default_dir = (
            os.path.dirname(self.sprite_path) if self.sprite_path
    ...
    def _browse_input_vram(self):
        ...
        suggested_dir = os.path.dirname(suggested_path) if suggested_path else settings.get_default_directory()
        ...
        if filename:
            ...
            settings.set_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_LAST_INPUT_VRAM, filename)
    ...
    def _browse_output_vram(self):
        ...
        if not suggested_path:
            if self.input_vram_edit.text():
                suggested_path = self._suggest_output_vram_path(self.input_vram_edit.text())
            else:
                # Use sprite directory if available
                sprite_dir = os.path.dirname(self.sprite_path) if self.sprite_path else settings.get_default_directory()
                sprite_base = os.path.splitext(os.path.basename(self.sprite_path))[0] if self.sprite_path else "sprite"
        ...
        if filename:
            ...
            settings.set_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_LAST_OUTPUT_VRAM, filename)
    ...
    def _load_rom_info(self, rom_path: str):
        ...
        try:
            ...
            logger.error(f"Failed to load ROM info from {rom_path}: {e}")
            QMessageBox.warning(self, "ROM Load Error", f"Failed to load ROM info: {e!s}")
    ...
    def get_parameters(self) -> Optional[dict]:
        ...
        if current_tab == 0:  # VRAM injection
            ...
            try:
                ...
                if offset_text.startswith(("0x", "0X")):
    ...
    def _set_initial_paths(self):
        ...
        if self.input_vram_edit.text():
            self.output_vram_edit.setText(self._suggest_output_vram_path(self.input_vram_edit.text()))
    ...
    def _set_rom_injection_defaults(self):
        ...
        if hasattr(self, 'rom_extraction_info') and self.rom_extraction_info:
            ...
            if rom_source and self.sprite_path:
                ...
                if os.path.exists(possible_rom_path):
                    ...
                    self.output_rom_edit.setText(self._suggest_output_rom_path(possible_rom_path))
        ...
        last_input_rom = settings.get_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_LAST_INPUT_ROM, "")
        ...
        last_custom_offset = settings.get_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_LAST_CUSTOM_OFFSET, "")
        if last_custom_offset:
            self.rom_offset_hex_edit.setText(last_custom_offset)

        # Load fast compression setting
        fast_compression = settings.get_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_FAST_COMPRESSION, False)
    ...
    def _restore_saved_sprite_location(self):
        ...
        last_sprite_location = settings.get_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_LAST_SPRITE_LOCATION, "")
    ...
    def _get_rom_offset_from_vram(self, vram_offset_str: str) -> Optional[int]:
        ...
        try:
            ...
            if isinstance(vram_offset_str, str):
                vram_offset = int(vram_offset_str, 16) if vram_offset_str.startswith(("0x", "0X")) else int(vram_offset_str, 16)
            else:
    ...
    def save_rom_injection_parameters(self):
        ...
        input_rom = self.input_rom_edit.text()
        if input_rom:
            settings.set_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_LAST_INPUT_ROM, input_rom)
        ...
        if sprite_location_text and sprite_location_text != "Select sprite location...":
            settings.set_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_LAST_SPRITE_LOCATION, sprite_location_text)

        # Save custom offset if used
        custom_offset = self.rom_offset_hex_edit.text()
        if custom_offset:
            settings.set_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_LAST_CUSTOM_OFFSET, custom_offset)

        # Save fast compression setting
        fast_compression = self.fast_compression_check.isChecked()
        settings.set_value(SETTINGS_NS_ROM_INJECTION, SETTINGS_KEY_FAST_COMPRESSION, fast_compression)

        # Save settings to file
        try:
            settings.save()
        except Exception as e:
            logger.error(f"Failed to save ROM injection parameters: {e}")
