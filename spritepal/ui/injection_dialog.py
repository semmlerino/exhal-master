...
class InjectionDialog(QDialog):
        ...
        if self.metadata_path and os.path.exists(self.metadata_path):
            try:
                ...
                if "extraction" in self.metadata:
                    ...
                    # Set default offset
                    self.offset_hex_edit.setText(
                        extraction.get("vram_offset", "0xC000")
                    )
                else:
    ...
    def _browse_input_vram(self):
        ...
        # Fall back to default directory if no sprite-specific directory found
        if not suggested_dir:
            suggested_dir = settings.get_default_directory()
    ...
    def _browse_output_vram(self):
        ...
        # If no suggested path, use default directory
        if not suggested_path:
            default_dir = settings.get_default_directory()
            suggested_path = os.path.join(default_dir, "modified_vram.dmp")
