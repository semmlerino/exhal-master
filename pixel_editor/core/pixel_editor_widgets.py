...
class ColorPaletteWidget(QWidget):
    ...
    def paintEvent(self, event: Optional[Any]) -> None:
...
class PixelCanvas(QWidget):
    ...
    def set_drawing_color(self, color_index: int):
        """Set the current drawing color (API compatibility method)"""
        self.current_color = color_index

    @property
    def drawing_color(self) -> int:
        """Get the current drawing color (API compatibility property)"""
        return self.current_color

    def create_new_image(self, width: int = 8, height: int = 8):
        """Create a new image (API compatibility method)"""
