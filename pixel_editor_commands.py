...
class DrawPixelCommand(UndoCommand):
    ...
    def execute(self, canvas: 'PixelCanvas') -> None:
        ...
        if (canvas.image_data is not None and 
            0 <= self.x < canvas.image_data.shape[1] and 
...
class DrawLineCommand(UndoCommand):
    ...
    pixels: List[Tuple[int, int, int]] = field(default_factory=list)  # [(x, y, old_color), ...]
    new_color: int = 0
...
class FloodFillCommand(UndoCommand):
    ...
    def execute(self, canvas: 'PixelCanvas') -> None:
        ...
        for dy in range(h):
            for dx in range(w):
                px, py = x + dx, y + dy
                if (0 <= px < canvas.image_data.shape[1] and 
                    0 <= py < canvas.image_data.shape[0]):
    ...
    def _get_compress_data(self) -> Tuple[Tuple[int, int, int, int], Optional[np.ndarray], int]:
    ...
    def _restore_from_compressed(self, data: Tuple[Tuple[int, int, int, int], Optional[np.ndarray], int]) -> None:
...
class UndoManager:
    ...
    def load_history(self, history: List[Dict[str, Any]], canvas: 'PixelCanvas') -> None:
