#!/usr/bin/env python3
"""
Corrected demonstration of Phase 1 improvements in the Pixel Editor.
...
class Phase1DemoWindow(QMainWindow):
    ...
    def demo_canvas_optimizations(self):
        ...
        # Force palette update to populate cache
        self.canvas._palette_version += 1
        self.canvas._update_qcolor_cache()
        
        start = time.time()
        for _ in range(1000):
            # Access cached colors - this is much faster than creating new QColors
            for i in range(16):
                color = self.canvas._qcolor_cache.get(i)
        ...
        # Simulate drawing a pixel
        self.canvas._mark_dirty(10, 10, 1, 1)
        if self.canvas._dirty_rect:
            rect = self.canvas._dirty_rect
            self.log(f"   - Single pixel edit updates only: QRect({rect.x()}, {rect.y()}, {rect.width()}, {rect.height()})")
    ...
    def demo_undo_system(self):
        ...
        # Create undo manager
        self.canvas.undo_manager = UndoManager()
        
        # Simulate drawing operations
        self.log("1. Memory Usage Comparison:")
        
        # Old system would store full images
        ...
        for i in range(50):
            ...
            self.canvas.undo_manager.execute_command(cmd)
        
        new_memory = self.canvas.undo_manager.get_memory_usage()
        ...
        # Test undo/redo
        self.log("\n2. Undo/Redo Test:")
        self.log(f"   - Can undo: {self.canvas.undo_manager.can_undo()} ({len(self.canvas.undo_manager.undo_stack)} operations)")
        self.log(f"   - Can redo: {self.canvas.undo_manager.can_redo()} ({len(self.canvas.undo_manager.redo_stack)} operations)")
        
        # Undo some operations
        for _ in range(5):
            if self.canvas.undo_manager.can_undo():
                self.canvas.undo_manager.undo()
        
        self.log(f"   - After 5 undos: {len(self.canvas.undo_manager.undo_stack)} to undo, {len(self.canvas.undo_manager.redo_stack)} to redo")
    ...
    def demo_workers(self):
        ...
        self.log("\n3. Starting worker (simulated)...")
        # Don't actually start it as it would try to load a real file
        self.log("   - Worker would run in background thread")
        self.log("   - UI remains fully responsive")
