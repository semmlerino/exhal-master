...
class PaletteController(BaseController):
    ...
    def __init__(self, sprite_model, palette_model, project_model, multi_palette_view, parent=None):
    ...
    def connect_signals(self):
        ...
        self.view.generate_preview_requested.connect(self.generate_multi_palette_preview)
    ...
    def browse_oam_file(self):
        ...
        initial_dir = ""
        if self.sprite_model.oam_file and os.path.exists(os.path.dirname(self.sprite_model.oam_file)):
        ...
        if file_name:
            ...
            if self.sprite_model.load_oam_mapping():
                QMessageBox.information(self.view, "Success", "OAM data loaded successfully")
            else:
                QMessageBox.warning(self.view, "Error", "Failed to load OAM data")
    
    def load_palettes(self):
        """Load all palettes from CGRAM"""
        if self.sprite_model.cgram_file:
            count = self.palette_model.load_palettes_from_cgram(self.sprite_model.cgram_file)
            return count > 0
        return False
    ...
    def generate_multi_palette_preview(self):
        ...
        if not self.sprite_model.vram_file:
            QMessageBox.warning(self.view, "Error", "Please load a VRAM file first")
            return
        
        if not self.sprite_model.cgram_file:
            QMessageBox.warning(self.view, "Error", "Please load a CGRAM file for palettes")
            return
