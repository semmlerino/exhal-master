...
class PaletteController(BaseController):
    ...
    def load_palettes(self):
        ...
        if self.sprite_model.cgram_file:
            count = self.model.load_palettes_from_cgram(self.sprite_model.cgram_file)
            return count > 0
        return False
    ...
    def generate_multi_palette_preview(self):
        ...
        if not self.sprite_model.cgram_file:
            QMessageBox.warning(
                self.view, "Error", "Please load a CGRAM file for palettes"
            )
            return
        ...
        try:
            ...
            if self.sprite_model.core.oam_mapper:
                ...
                self.model.set_oam_statistics(oam_stats)
            ...
            # Load all palettes
            self.load_palettes()
            palettes = self.model.get_all_palettes()
            ...
            QMessageBox.critical(
                self.view, "Error", f"Failed to generate preview: {e!s}"
            )
    ...
    def on_palette_selected(self, palette_num):
        ...
        if self.sprite_model.current_image:
            if self.model.apply_palette_to_image(
    ...
    def export_palette(self, palette_index, format="act"):
        ...
        if file_name:
            data = self.model.export_palette(palette_index, format)
            if data:
                ...
                QMessageBox.information(
                    self.view, "Success", f"Palette exported to {file_name}"
                )
            else:
                QMessageBox.warning(self.view, "Error", "Failed to export palette")
