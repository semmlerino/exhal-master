...
try:
    ...
    from .security_utils import SecurityError, validate_file_path, validate_output_path
...
class SpriteEditorCore:
    ...
    def extract_sprites(
        ...
        try:
            ...
            total_tiles, tiles_x, tiles_y, width, height = (
                self._calculate_sprite_dimensions(len(data), tiles_per_row)
            )
    ...
    def _extract_tile_from_image(
        self, pixels: list[int], tile_x: int, tile_y: int, img_width: int
    ) -> list[int]:
    ...
    def load_oam_mapping(self, oam_file: str) -> bool:
        ...
        try:
            ...
            self.logger.exception(f"Error loading OAM: {e}")
            return False

    def extract_sprites_multi_palette(
        self,
        vram_file: str,
        offset: int,
        size: int,
        ...
        if cgram_file:
            try:
                ...
                if not os.path.exists(cgram_file):
                    self.logger.warning(
                        f"CGRAM file not found: {cgram_file}, using grayscale palettes"
                    )
                    cgram_file = None
            except SecurityError:
                self.logger.exception(
                    f"Security error accessing CGRAM file: {cgram_file}"
                )
                cgram_file = None
        ...
        if cgram_file:
            for i in range(16):
                try:
                    ...
                    if (
                        pal and len(pal) >= 48
                    ):  # Ensure palette has at least 16 colors (16*3 RGB values)
                        palettes.append(pal)
                    else:
                        self.logger.warning(
                            f"Invalid palette {i} in CGRAM, using grayscale"
                        )
                        palettes.append(get_grayscale_palette())
                except Exception as e:
                    self.logger.warning(
                        f"Error reading palette {i}: {e}, using grayscale"
                    )
                    palettes.append(get_grayscale_palette())
        else:
            # Use grayscale if no CGRAM
    ...
    def create_palette_grid_preview(
        self,
        vram_file: str,
        offset: int,
        size: int,
        ...
        try:
            ...
            for pal_num in range(16):
                ...
                border_color = (
                    (0, 255, 0) if pal_num in active_palettes else (64, 64, 64)
                )
                ...
                # Draw label text
                text_color = (
                    (0, 255, 0) if pal_num in active_palettes else (255, 255, 255)
                )
                draw.text((img_x + 4, img_y + 4), label, fill=text_color)

            return grid_img, total_tiles

        except (OSError, RuntimeError, AttributeError, SecurityError) as e:
    ...
    def _validate_palette_index(self, palette_num: int) -> int:
        ...
        if not isinstance(palette_num, int):
            raise ValueError(
                f"Palette number must be an integer, got {type(palette_num)}"
            )
        if palette_num < 0:
            self.logger.warning(
                f"Palette number {palette_num} is negative, clamping to 0"
            )
            return 0
        if palette_num > 15:
            self.logger.warning(
                f"Palette number {palette_num} exceeds maximum (15), clamping to 15"
            )
            return 15
        return palette_num
