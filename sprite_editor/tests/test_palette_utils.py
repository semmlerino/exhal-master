...
from sprite_editor.palette_utils import (
    ...
    read_cgram_palette, get_grayscale_palette,
    read_all_palettes, write_cgram_palette
)
...
class TestColorConversion:
    ...
    def test_conversion_roundtrip_accuracy(self):
        ...
        for original_bgr555 in test_values:
            ...
            assert diff <= 0x0842, f"Too much precision loss: {original_bgr555:04X} -> {converted_bgr555:04X}"
...
class TestGrayscalePalette:
    ...
    def test_get_grayscale_palette_size(self):
        ...
        assert len(palette) == PALETTE_SIZE_BYTES  # 768 values (256 colors * 3)
...
class TestCgramPaletteReading:
    ...
    def test_read_cgram_palette_file_too_small(self, temp_dir):
        ...
        # Try to read palette 15 (should need offset 480, but file is only 256 bytes)
    ...
    def test_read_cgram_palette_incomplete_palette_data(self, temp_dir):
        ...
        # Implementation requires full palette (32 bytes), so should return None
...
class TestPaletteWriting:
    ...
    def test_write_cgram_palette_roundtrip(self, temp_dir):
        ...
        for i in range(48):
            ...
            assert diff <= 8, f"Color component {i}: {original_palette[i]} vs {read_palette[i]}"
