...
from sprite_editor.tile_utils import (
    decode_4bpp_tile, encode_4bpp_tile, 
    decode_tiles, encode_tiles
)
...
class TestTileDecoding:
    ...
    def test_decode_4bpp_tile_basic(self, sample_4bpp_tile):
        ...
        for y in range(TILE_HEIGHT):
            for x in range(TILE_WIDTH):
                ...
                if x == y:
                    assert tile_pixels[pixel_idx] == 1  # Diagonal should be color 1
                else:
    ...
    def test_decode_4bpp_tile_complex_pattern(self):
        ...
        for y in range(8):
            tile_data[y * 2] = 0xFF if y % 2 == 0 else 0x00      # bp0: alternating rows
            tile_data[y * 2 + 1] = 0x0F if y < 4 else 0xF0       # bp1: top/bottom half
            tile_data[16 + y * 2] = 0xAA                          # bp2: checkerboard
            tile_data[16 + y * 2 + 1] = 0x55                      # bp3: inverse checkerboard
